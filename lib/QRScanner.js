...
const TORCH_MODE_INIT_ON_DELAY = 500;
...
export default class QRScannerView extends Component {
    static propTypes = {
        ...
        torchMode: React.PropTypes.oneOf('on', 'off'),
    };
    ...
    constructor(props) {
        ...
        this.state = {
            torchModeInitOnDefered: props.torchMode !== 'on', //通过延缓初始化时设置闪光灯打开来规避react-native-camera的问题
        }
        this.torchModeDeferer = null;
        this.getTorchMode = this.getTorchMode.bind(this);
        this.deferTorchInitOnIfNecessary = this.deferTorchInitOnIfNecessary.bind(this);
    }

    componentDidMount() {
        this.deferTorchInitOnIfNecessary();
    }

    deferTorchInitOnIfNecessary() {
        if (!this.state.torchModeInitOnDefered && !this.torchModeDeferer) {
            const deferer = setTimeout(() => {
                this.setState({
                    torchModeInitOnDefered: true,
                })
            }, TORCH_MODE_INIT_ON_DELAY);
            this.torchModeDeferer = deferer;
        }
    }

    getTorchMode() {
        switch (this.props.torchMode) {
            case 'on':
                if (this.state.torchModeInitOnDefered) {
